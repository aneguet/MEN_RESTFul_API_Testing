...
describe("/General, User and Artist tests", function () {
  ...
  //       // FIXME console.log("Body response: " + actualVal);
  ...
  // });
  // ------------------------------------------------------------------------
  // Check that artist name doesn't exist already (Apply joi to artist routes file) !!!!!!!!!!!!!
  // ----- User Tests
  ...
  //               expect(res.body.length).to.be.eql(1);
  //               let savedArtist = res.body[0];
  //               // FIXME console.log("********* SAVED ARTIST ID: " + savedArtist._id);
  //               expect(savedArtist.name).to.be.equal(artist.name);
  //               expect(savedArtist.info).to.be.equal(artist.info);
  //               expect(savedArtist.genre).to.be.equal(artist.genre);
  //               expect(savedArtist.photo).to.be.equal(artist.photo);
  ...
  //                 .end((err, res) => {
  //                   res.should.have.status(200);
  //                   // FIXME console.log("*********** " + res.body.name);
  //                   expect(res.body).to.be.a("object");
  //                   // expect(res.body.length).to.be.eql(1); // ??????????????
  ...
  //                   // FIXME console.log("*********** " + res.body.message);
